...
public class PlayerSettings extends JDialog implements ActionListener {
    ...
    JRadioButton red1 = new JRadioButton("Piros", true);
    JRadioButton green1 = new JRadioButton("Zöld");
    JRadioButton blue1 = new JRadioButton("Kék");
    JRadioButton yellow1 = new JRadioButton("Sárga");

    ///2. játékos beállítások létrehozása
    JPanel player2Settings = new JPanel();
    JLabel player2Label = new JLabel("2. Játékos (Jobb alsó sarok, Nyilak):");
    ...
    JRadioButton red2 = new JRadioButton("Piros", true);
    JRadioButton green2 = new JRadioButton("Zöld");
    JRadioButton blue2 = new JRadioButton("Kék");
    JRadioButton yellow2 = new JRadioButton("Sárga");
    ...
    JTextArea settingsLog = new JTextArea("Állítsd be a játékosok adatait!" + System.lineSeparator(), 4, 50);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        for (JRadioButton currentButton : player1ColorOptions) {
            if (currentButton.isSelected()) {
                if(currentButton.getText().equals("Piros")){
                    givenPlayer1Color = PlayerColor.RED;
                } else if(currentButton.getText().equals("Zöld")){
                    givenPlayer1Color = PlayerColor.GREEN;
                } else if(currentButton.getText().equals("Kék")){
                    givenPlayer1Color = PlayerColor.BLUE;
                } else if(currentButton.getText().equals("Sárga")){
        ...
        if (givenPlayer1Name.trim().length() > 0
            ...
            settingsLog.setText("");
            if (givenPlayer1Name.trim().length() == 0) {
                settingsLog.append("Hiba: 1. játékos neve üres." + System.lineSeparator());
            }
            if (givenPlayer2Name.trim().length() == 0) {
                settingsLog.append("Hiba: 2. játékos neve üres." + System.lineSeparator());
            }
            if (givenPlayer1Name.equals(givenPlayer2Name)) {
                settingsLog.append("Hiba: A két játékos neve megegyezik." + System.lineSeparator());
            }
            if (givenPlayer1Color == givenPlayer2Color) {
                settingsLog.append("Hiba: A két játékos színe megegyezik" + System.lineSeparator());
            }
            if (givenPlayer1Name.contains(";") || givenPlayer2Name.contains(";")) {
                settingsLog.append("Hiba: A játékosnevekben nem lehet ; karakter" + System.lineSeparator());
            }
